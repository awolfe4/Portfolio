...
# Load data
WR_with_Salary_REAL <- read.csv('wr_salary_stats_24.csv')
WR_with_Salary <- read.csv('wr_salary_stats_24.csv')

# Step 1: Remove non-numeric columns (e.g., player_name, Pos.)
data <- WR_with_Salary %>%
  select(-player_name,-racr,-yards_per_catch,-Expected_Yards_After_Catch_PlusMinus,-catches,-Average_Cushion)
...
# Step 15: Predictions for the entire dataset using Random Forest model
X_scaled <- predict(scaler, data %>% select(-APY, -log_APY))

# Predict for all players in the dataset
rf_all_pred_log <- predict(rf_model, X_scaled)
rf_all_pred <- expm1(rf_all_pred_log)  # Inverse of log1p

# Step 16: Add predicted values back to the original dataset
WR_with_Salary_REAL$RF_Predicted_APY <- rf_all_pred

# Step 17: Calculate differences (underpayment or overpayment) for each model
WR_with_Salary_REAL$RF_Difference <- WR_with_Salary_REAL$APY - WR_with_Salary_REAL$RF_Predicted_APY

# Step 18: Show the top 10 players who are underpaid or overpaid based on Random Forest predictions
top_3_overpaid <- WR_with_Salary_REAL %>%
  arrange(desc(RF_Difference)) %>%
  head(3) %>%
  select(player_name, APY, RF_Predicted_APY, RF_Difference)

top_3_underpaid <- WR_with_Salary_REAL %>%
  arrange(RF_Difference) %>%
  head(5) %>%
  select(player_name, APY, RF_Predicted_APY, RF_Difference)

# Optionally: Show the updated dataset with all predictions and differences
print(head(WR_with_Salary_REAL %>%
  select(player_name, APY, RF_Predicted_APY, RF_Difference))) 

# Step 19: Export the top 10 players to CSV
#write.csv(top_3_overpaid, 'top_3_overpaid_players_wr.csv', row.names = FALSE)
#write.csv(top_3_underpaid, 'top_3_underpaid_players_wr.csv', row.names = FALSE)
```

```{r}
# Step 1: Predict for the entire dataset using Random Forest model
X_scaled <- predict(scaler, data %>% select(-APY, -log_APY))

# Predict for all players in the dataset using the Random Forest model
rf_all_pred_log <- predict(rf_model, X_scaled)
...
# Step 4: Show the top 10 players who are underpaid or overpaid based on Random Forest predictions
top_3_overpaid_rf <- WR_with_Salary_REAL %>%
  arrange(desc(RF_Difference)) %>%
  head(2) %>%
  select(player_name, APY, RF_Predicted_APY, RF_Difference)

top_3_underpaid_rf <- WR_with_Salary_REAL %>%
  arrange(RF_Difference) %>%
  head(2) %>%
  select(player_name, APY, RF_Predicted_APY, RF_Difference)
...
#write.csv(top_3_overpaid_rf, 'top_2_overpaid_players_rf_wr.csv', row.names = FALSE)
#write.csv(top_3_underpaid_rf, 'top_2_underpaid_players_rf_Wr.csv', row.names = FALSE)

# Step 6: Export the entire updated dataset with projected salaries and differences to CSV
#write.csv(WR_with_Salary_REAL, 'WR_with_Salary_with_Projected_RF.csv', row.names = FALSE)
```

```{r}
# Combine the top 3 overpaid and underpaid players into one dataset
top_3_combined <- bind_rows(
  top_3_underpaid_rf %>% mutate(Salary_Status = "Overpaid"),
  top_3_overpaid_rf %>% mutate(Salary_Status = "Underpaid")
)
...
top_3_combined %>%
  ...
  labs(title = "Top 2 Overpaid and Underpaid WRs", 
       x = "Player", 
       y = "Difference (Actual - Predicted) in Millions of $", 
