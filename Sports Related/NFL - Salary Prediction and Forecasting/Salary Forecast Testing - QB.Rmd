...
# Load data
QB_with_Salary_REAL <- read.csv('qb_salary_stats_24_2.csv')
QB_with_Salary <- read.csv('qb_salary_stats_24_2.csv')

# Step 1: Remove non-numeric columns (e.g., player_name, Pos.)
data <- QB_with_Salary %>%
  select(-player_name, -Pos.,-rushing_first_downs,-pass_epa,-rushing_epa,-Age)
...
set.seed(123)
trainIndex <- createDataPartition(data$log_APY, p = 0.8, list = FALSE)
...
top_5_overpaid_rf <- QB_with_Salary_REAL %>%
  arrange(desc(RF_Difference)) %>%
  head(3) %>%
  select(player_name, APY, RF_Predicted_APY, RF_Difference)

top_5_underpaid_rf <- QB_with_Salary_REAL %>%
  arrange(RF_Difference) %>%
  head(3) %>%
  select(player_name, APY, RF_Predicted_APY, RF_Difference)
...
# Step 5: Export the entire updated dataset with projected salaries and differences to CSV
#write.csv(QB_with_Salary_REAL, 'QB_with_Salary_with_Projected_RF_qb.csv', row.names = FALSE)
...
top_5_combined <- bind_rows(
  top_5_underpaid_rf %>% mutate(Salary_Status = "Overpaid"),
  top_5_overpaid_rf %>% mutate(Salary_Status = "Underpaid")
)
...
top_5_combined %>%
  ...
  labs(title = "Top 3 Overpaid and Underpaid QBs", 
       x = "Player", 
